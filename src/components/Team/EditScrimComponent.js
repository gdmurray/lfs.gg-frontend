        ...
        if (!this.state.editMode) {
            ...
            return (
                <Menu.Menu position='right'>
                    ...
                    <Menu.Item onClick={() => this.setState({ editMode: false })} as='a' icon='times circle outline' color='red' />
                </Menu.Menu>
            )
        }
    }
    ...
    getField = (name) => {
        ...
        if (name === 'request_notes') {
            if (!editMode) {
                ...
                return (<input onChange={this.handleInputChange} name='notes' value={scrim.request_notes ? scrim.request_notes : ''} />)
            }
        }else if(name === 'time'){
            if(!editMode){
                ...
                return(<DateTimeInput timeFormat='ampm' onChange={this.handleInputChange} name='time' value={scrim.time} />)
            }
        }
}
            ...
            <div style={{ marginBottom: '10px' }}>
                <Menu attached='top' className='edit-scrim-menu'>
                    <Menu.Item className='edit-scrim-menu-label'>
                        <Label content='Looking' color='yellow' />
                    </Menu.Item>
                    {this.getIcons()}
                </Menu>
